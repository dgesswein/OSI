...
; OSI 505, 470 board I/O reference
; C000-C003 = 6821 PIA (decoded to C000-C00F) (C000/C040/C080/C0C0)
; C010-C011 = 6850 ACIA (Decode to C010-C01F) (C010/C050/C090/C0D0)
...
; PB2 -> !STEP DIRECTION    (0 = to trk 39/76)
; PB3 -> !STEP              (step on 1->0 transition)
; PB4 -> !FAULT RESET (nc)  (leave 1) (motor on?)
; PB5 -> Drive Enable       (1 = drive ON) (Or Drive/Side sel. 4 drive system 1=drive 0&1,0=drive 2&3 )
; PB6 -> !LOW CURRENT (nc)  (leave 1 on 5.25; On 8" disk 0 when writing trk 44+)
...
; CB2 <- ?
;
; Disk routines set port A to all input except PA6, port B to all output
...
LF      =   $0A
ESC     =   $1B     ; ESC to exit
ACIA8N1 =   $15     ;6850 ACIA control reg setting for 8N1
ACIA8N2 =   $11     ;6850 ACIA control reg setting for 8N2 no irq (OSI default C1)
ACIA8N2i =  $B1     ;6850 ACIA control reg setting for 8N2 +irq (OSI default C3)
...
TMP         = STORE+06 ;1
MACHINE     = STORE+07 ;1 C1/C2/C3 flag 00=C2/4/8, $40=C3Ser, $80=C1
INVKEYB     = STORE+08 ;C1 polled = FF C2 polled = 00

VIDEO       = STORE+09 ;1 0 = Serial $FF = video
PATSEL      = STORE+10 ;1  0 = Fixed byte (PATVAL), $FF = random
PATVAL      = STORE+11 ;1  Byte to write to disk
PASSES      = STORE+12 ;1
READONLY    = STORE+13 ;1 .BYTE 0 ; zero read write
PASSCNTR    = STORE+14 ;1 .BYTE 0
RND         = STORE+15 ;6 .BYTE 92, 159, 137, 36, 210, 89
RNDHLD      = STORE+21 ;6 .BYTE 0,0,0,0,0,0
DIVIDEND    = STORE+27 ;3 .BYTE 0,0,0
DIVISOR     = STORE+30 ;3 .BYTE 0,0,0
REMAINDER   = STORE+33 ;3 .BYTE 0,0,0
factor2     = STORE+36 ;3 .BYTE 0,0,0
PZTEMP      = STORE+39 ;1 .BYTE 0
MAX         = STORE+40 ;3 .BYTE 0,0,0
MIN         = STORE+43 ;3 .BYTE 0,0,0
SUM         = STORE+46 ;3 .BYTE 0,0,0
pad         = STORE+49 ;1 .BYTE 0
ERRCNT      = STORE+50 ;2 .WORD 0
PRTERR      = STORE+52 ;1 .BYTE 0

; Allow 3% fast rotation and 1.25 delay after index high and 2.2 ms index
; pulse;
MTRKBYTES   = STORE+53 ;2 .WORD 0   ; negative number of bytes in track
BYTECNTR    = STORE+55 ;2 .WORD 0
YHOLD       = STORE+57 ;1 .BYTE 0
TESTTYPE    = STORE+58 ;1 .BYTE 0   ; 0 normal 1 scope

;zero page storage
DRVACIA     =STORE+60 ;1 .BYTE 0 ; Current serial word format for DISK ACIA 8E1/8N1
PTRSTOR     =STORE+61 ;2 .WORD 0 
TDATA       =STORE+63 ;2 .WORD 0
TDATA2      =STORE+65 ;2 .WORD 0
VIDSRC      =STORE+67 ;2 .WORD $D040
VIDDST      =STORE+69 ;2 .WORD $D000
VIDOFFSET   =STORE+71 ;1 .BYTE 0
VIDXLEN     =STORE+72 ;1 .BYTE 0
VIDXMAX     =STORE+73 ;1 .BYTE 0
MAXERR      =STORE+74 ;1 .BYTE 0 ; number of errors + 1 to print
SINGLETRK   =STORE+75 ;1 .BYTE 0; non zero if testing single track

	; Serial monitor start values
	; (now emitted by A65 65A output format
	;*= $129
...
; INIT DISK CONTROLLER PIA returns ZERO if no controller found
INITPIA 
	LDY #0
	LDA #$40
...
DRWMENU
	...
	BNE DRWMNK
	JSR PRINT
	.BYTE $D,$A,$D,$A,'** NO DISK CTRL DETECTED **',0
DRWMNK
	 JSR PRINT
	.BYTE $D,$A,$D,$A
	.BYTE 'OSI DESTRUCTIVE DiskTest',$D,$A          
	.BYTE '------------------------',$D,$A          
	.BYTE '1. Test Disk',$D,$A
	.BYTE '2. Test Track',$D,$A
	.BYTE '3. Select Drive',$D,$A      
	.BYTE '4. Set Drive Type',$D,$A      
	.BYTE '5. Set Pattern & passes',$D,$A
...
Get_Chr_Polled
	...
	BCC NOKEY       ;no, dont get stuck in keypoller
	JSR $FEED       ;yes - check keypoller
	PHA
	LDA #$00
	STA $0215       ;clear keypoller last key
UNKEY
...
TOAC1
	...
	; FFE0 CC, 2F, 00 (C1E cegmon 64x16)
VIDINIT
	LDA #$00
	STA VIDOFFSET
	STA VIDDST
	...
	LDX $FFE2
	BEQ C1VID       ;1k screen
	CPX #$8D		;NewMon UK101 $FB85 =  len
	BEQ NEWMON
	CPX #$AA
	BEQ WEMON
	DEX
	BEQ C2VID
	CPX #$8C   ;newmon is 8D@FFE2
	BEQ NEWMON
	CPX #$A9
	BEQ WEMON  ;wemon is AA@FFE2
